...
def test_layout_circuit_rejects_engine_and_config_together() -> None:
    ...
    with pytest.raises(BuildParameterError, match="only one of engine= or config="):
        layout_circuit(
            result,
            engine=LayoutEngine(LayoutConfig(cell_gap=0.6)),
            config=LayoutConfig(cell_gap=5.0),
        )
