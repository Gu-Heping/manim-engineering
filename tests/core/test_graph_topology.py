...
core types only (``CircuitGraph``, ``Node``, ``Connection``, ``ConnectionState``,
``PinDirection``, ``SignalType``, ``InvalidConnectionError``, ``InvalidPortError``).
Previously lived in ``tests/semantic/test_graph.py``; moved as part of the
A+B+D follow-up de-duplication (E-2).
"""
