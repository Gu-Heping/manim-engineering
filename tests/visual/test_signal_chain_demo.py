"""Golden visual regression for examples/basics/signal_chain_demo.SignalChainDemo."""
...
_GOLDEN_NAME = "signal_chain_demo.dhash.txt"
_EXAMPLE = Path(__file__).resolve().parents[2] / "examples" / "basics" / "signal_chain_demo.py"


def _load_module():
    spec = importlib.util.spec_from_file_location("signal_chain_demo", _EXAMPLE)
...
def _render_last_frame_png() -> Path:
    ...
    with tempconfig(
        ...
        mod.SignalChainDemo().render()

    matches = sorted(Path(tmpdir).rglob("SignalChainDemo*.png"))
    if not matches:
        raise FileNotFoundError(f"no SignalChainDemo PNG under {tmpdir}")
    return matches[-1]


def _geometry_hash_from_fixture() -> str:
    mod = _load_module()
    _graph, _elements, layout, _net12, _net23, bundle = mod.build_signal_chain_fixture()
    return layout_waveform_geometry_digest(layout, bundle)


@requires_manim
def test_signal_chain_demo_last_frame_phash() -> None:
...
def test_signal_chain_demo_geometry_hash_recorded() -> None:
    """Point geometry hash is stable (layout + waveform panel)."""
    digest = _geometry_hash_from_fixture()
    geom_path = _GOLDEN_DIR / "signal_chain_demo.geometry.txt"
