...
2. **Entry**: no bare `self.add(topology, hud)` in the first frame. Call `play_topology_intro(...)` (or override `WaveformDemoScene.play_intro()`) from `manim_engineering.animation` — `Create` on `Line` symbol strokes, `DrawBorderThenFill` on filled `Polygon`/`Dot` bodies when `IntroStyle.use_border_fill` is enabled; hide labels during body reveal, then `show_labels` + `normalize_topology_labels`. **Never** `FadeIn(topology.components)` or `topology.components.set_opacity(1.0)` on mixed groups (activates white fill on Manim `Line` symbols). HUD: `play_hud_intro` uses `FadeIn(title)` (CJK-stable) + `FadeIn(intro, shift=…)`.
